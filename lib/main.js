...
var app = new Vue({
  ...
  data: {
    ...
    timer: '',
    plot_distance_layout: {'title': 'One Way Communication Latency History',
    'yaxis_title': 'One Way Communication Latency (Minutes)', 
    'xaxis_title': 'Date' },
    plot_distance_layout_cn: {'title': '地球-火星单向通信延时',
  ...
  methods: {
    ...
    plot_distance: function () {
      ...
      if (cnelementExists) {
      plot_mars_earth_distance_history(this.mars,this.current_date, this.plot_distance_layout_cn.title, this.plot_distance_layout_cn.xaxis_title, this.plot_distance_layout_cn.yaxis_title, 'distance-chart-cn')
    } else if (enelementExists) {
      plot_mars_earth_distance_history(this.mars,this.current_date, this.plot_distance_layout.title, this.plot_distance_layout.xaxis_title, this.plot_distance_layout.yaxis_title, 'distance-chart')
    }
  ...
  computed: {
    ...
    current_date_string: function () {
      return moment(this.current_date).format('MM/DD/YYYY hh:mm')
    },
    current_date_string_cn: function () {
      return moment(this.current_date).locale("zh-cn").format('LLL')
    },
